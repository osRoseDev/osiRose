...
void CLoginServer::DeleteClientSocket( CClientSocket* thisclient )
{
     try
     {
       	Log( MSG_INFO, "User Disconnected from Login Server." );
...
// Load Server configuration
{
     try
     {
    	...
    	Config.SQLServer.pcDatabase = ConfigGetString ( file, "mysql_database", "irose" );
    ...
    {
        Log(MSG_FATALERROR, "Error parsing configuration file 'loginserver.conf'");
    }
}
...
bool CLoginServer::OnReceivePacket( CClientSocket* thisclient, CPacket *P )
{
	switch( P->Command )
	{
	   ...
	   case 0x703: return pakEncryptionRequest( (CLoginClient*)thisclient, P );     // Encryption Request.
	   case 0x708: return pakUserLogin        ( (CLoginClient*)thisclient, P );     // User Login
	   case 0x704: return pakGetServers       ( (CLoginClient*)thisclient, P );     // Get Server List
	   case 0x70a: return pakGetIP            ( (CLoginClient*)thisclient, P );     // Get Server IP
